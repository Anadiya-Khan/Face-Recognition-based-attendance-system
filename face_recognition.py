...
class face_recognition1:
    def __init__(self,root):
        ...
        title_lbl=Label(self.root,text="Face Recognition of Student",font=("times new roman",30,"bold"),fg="blue")
    ...
    '''def mark_attendance(self, student_id,roll_no, name, department):
        ...
        with open("attendance_report/std.csv", "r+", newline="\n") as f:
            ...
            if entry[0] == student_id and entry[4] == date_str:
                ...
                return
            else:# If the student doesn't have attendance for today, write a new record
              f.write(f"{student_id},{roll_no},{name},{department},{time_str},{date_str},Present\n")
    ...
    def mark_attendance(self, student_id, roll_no, name, department):
        ...
        # Read all the lines in the CSV file
        with open("attendance_Student/std.csv", "r") as f:
            myDatalist = f.readlines()  # Read all lines from the file

            # Check if the student has already marked attendance today
            for line in myDatalist:
                entry = line.split(",")  # Assuming CSV format: student_id, roll_no, name, department, time, date, status
                if entry[0] == student_id and entry[5] == date_str:
        ...
        # If attendance is not marked yet, append the new attendance
        with open("attendance_Student/std.csv", "a") as f:
            f.write(f"{student_id},{roll_no},{name},{department},{time_str},{date_str},Present\n")
            print(f"Attendance for {student_id} on {date_str} marked successfully.")

        # Mark the student as attended today in the session
        self.attendance_marked_today.add(student_id)    
    def face_reco(self):
            ...
            for (x, y, w, h) in features:
                ...
                with mysql.connector.connect(host="localhost", username="root", password="Anadiya@786", database="face_recognition") as conn:
                    ...
                    def fetch_single_value(query):
                        ...
                        return result[0] if result else "Nahi pata" # If no result, return "N/A"

                    # Get student details if result else "N/A"
                    student_id = fetch_single_value(f"SELECT Student_id FROM student WHERE Student_id="+str(id))
                    roll_no = fetch_single_value(f"SELECT Rollno FROM student WHERE Student_id="+str(id))
                    name = fetch_single_value(f"SELECT Name FROM student WHERE Student_id="+str(id))
                    department = fetch_single_value(f"SELECT Department FROM student WHERE Student_id="+str(id))
                    '''teacher_id = fetch_single_value(f"SELECT ID from teacher WHERE ID="+str(id))
                    t_name = fetch_single_value(f"SELECT Name from teacher WHERE ID="+str(id))
                    dep = fetch_single_value(f"SELECT Department from teacher WHERE ID="+str(id))'''

                # Draw the recognized information
                if confidence > 77:
                    #if student_id:
                    cv2.putText(img, f"Id: {student_id}", (x, y - 75), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                    cv2.putText(img, f"Rollno: {roll_no}", (x, y - 55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                    ...
                    self.mark_attendance(student_id, roll_no, name, department)
                    #elif teacher_id:
                        #cv2.putText(img, f"ID:{teacher_id}", (x, y - 75), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                        #cv2.putText(img, f"Name :{t_name}", (x, y - 55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                        #cv2.putText(img, f"Department:{dep}", (x, y - 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                        #self.mark_attendance_teacher(teacher_id, t_name, dep)  '''  
                else:
        ...
        clf.read("classifier_student.xml")
